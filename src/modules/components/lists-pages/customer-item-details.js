...
const Item = props => (
  <tr>
    ...
    <td>
      <Link to={"/edititem/" + props.userid + "/" + props.item._id}>edit</Link>
    </td>
  </tr>
)

export default class CustomersItemsList extends Component {
  constructor(props) {
    super(props);

    this.deleteItem= this.deleteItem.bind(this)

    this.state = {items: [], userid:'' };
  }
 

  componentDidMount() {
    const x = this.props.match.params.id;
    axios.get('http://localhost:5000/users/item/'+ x)
      .then(response => {
        this.setState({ 
          items: response.data, 
          userid: x 
  ...
  itemsList() {
    return this.state.items.map(currentitem => {
      return <Item item={currentitem} userid= {this.state.userid} deleteitem={this.deleteItem} key={currentitem._id}/>;
    })
  }
